...
#define ETRACE() do { bool success = true; word_t a7 = isa_reg_str2val("a7",&success); assert(success);  etrace(read_csrs(CSR_MEPC), read_csrs(CSR_MCAUSE), a7, read_csrs(CSR_MTVEC));printf("etrace ok\n");} while(0)
#define ECALL(dnpc) do { dnpc = isa_raise_intr(ENVIRONMENT_CALL_FROM_U_MODE, s->pc); ETRACE();} while(0)
